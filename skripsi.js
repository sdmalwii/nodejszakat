...
app.put('/api/tbl_penerimaan/:Id_Penerima', (req, res) => {
  const { Id_Mustahik, Nama, Alamat, Jenis_Kelamin, Golongan, Usia, Jumlah_Beras, Tanggal_Terima } = req.body;
  
  const querySearch = 'SELECT * FROM tbl_penerimaan WHERE Id_Penerima = ?';
  const queryUpdate = 'UPDATE tbl_penerimaan SET Id_Mustahik=?, Nama=?, Alamat=?, Jenis_Kelamin=?, Golongan=?, Usia=?, Jumlah_Beras=?, Tanggal_Terima=? WHERE Id_Penerima=?';

  koneksi.query(querySearch, [req.params.Id_Penerima], (err, rows) => {
    ...
    if (!rows.length) return res.status(404).json({ 
      success: false, 
      message: 'Data tidak ditemukan' + req.params.Id_Penerima 
    });

    koneksi.query(queryUpdate, [
      Id_Mustahik, Nama, Alamat, Jenis_Kelamin, 
      Golongan, Usia, Jumlah_Beras, Tanggal_Terima, 
      req.params.Id_Penerima
    ], (err, result) => {
      if (err) return res.status(500).json({ 
        success: false, 
      ...
      res.status(200).json({ 
        ...
        updatedId: req.params.Id_Penerima
      });
    });
  });
});

app.delete('/api/tbl_penerimaan/:Id_Penerima', (req, res) => {
  const querySearch = 'SELECT * FROM tbl_penerimaan WHERE Id_Penerima = ?';
  const queryDelete = 'DELETE FROM tbl_penerimaan WHERE Id_Penerima = ?';
  ...
  koneksi.query(querySearch, [req.params.Id_Penerima], (err, rows) => {
    ...
    koneksi.query(queryDelete, [req.params.Id_Penerima], (err, result) => {
      if (err) return res.status(500).json({ 
        success: false, 
      ...
      res.status(200).json({ 
        ...
        deletedId: req.params.Id_Penerima
      });
    });
  });
});
